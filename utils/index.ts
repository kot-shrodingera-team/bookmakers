 ...
 */
const log = (message: string, color = 'white', dev = false): void => {
  if (dev && !worker.Dev) {
    return;
  }
...
export const trim = (string: string, short = false): string => {
  ...
  return string
    ...
    .replace(/\n+/g, '\n') // "сжимание" последовательностей переносов до одного переноса
...
export const germesLog = (
  message: string,
  logType: LogType = LogType.CUSTOM,
  color = 'white',
  dev = false,
): void => {
  const data = (() => {
    switch (logType) {
      ...
      case LogType.DEV_INFO:
        return {
          color: 'cadetblue',
          dev: true,
        };
      case LogType.DEV_ACTION:
        return {
          color: 'darksalmon',
          dev: true,
        };
  ...
  log(message, data.color, data.dev);
};

export const getSessionData = (key: string): string => {
  const data = worker.GetSessionData(
    `${window.germesData.bookmakerName}.${key}`,
  );
  if (data === null) {
    return '';
  }
